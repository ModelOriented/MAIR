...
def fetch_paper_information(arxiv_id):
    ...
    time.sleep(2.5)
    paper = sch.paper('arXiv:{}'.format(arxiv_id), timeout=10, include_unknown_references=True)
    try:
        return paper, [p['arxivId'] for p in paper['references'] if p['arxivId'] is not None]
    except KeyError:
        return paper, []
